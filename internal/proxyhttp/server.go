...
// RateLimit is the latest upstream quota snapshot, captured from the
// X-Ratelimit-* response headers. Updated on every proxied request.
...
// RateLimit returns the latest upstream quota snapshot captured from
// X-Ratelimit-* response headers. Returns HasData=false until the first
// proxied request completes.
func (s *Server) RateLimit() RateLimit {
        s.rlMu.RLock()
        defer s.rlMu.RUnlock()
        return s.rl
}

// captureRateLimit reads X-Ratelimit-* headers from an upstream response and
// stores them as the latest quota snapshot. Called after every proxied request.
func (s *Server) captureRateLimit(h http.Header, model string) {
        rl := RateLimit{
                LimitRequests:     parseI64(h.Get("X-Ratelimit-Limit-Requests")),
                RemainingRequests: parseI64(h.Get("X-Ratelimit-Remaining-Requests")),
                LimitTokens:       parseI64(h.Get("X-Ratelimit-Limit-Tokens")),
                RemainingTokens:   parseI64(h.Get("X-Ratelimit-Remaining-Tokens")),
                ResetRequests:     h.Get("X-Ratelimit-Reset-Requests"),
                ResetTokens:       h.Get("X-Ratelimit-Reset-Tokens"),
                LastModel:         model,
                LastUpdated:       time.Now().UTC().Format(time.RFC3339),
                HasData:           h.Get("X-Ratelimit-Limit-Requests") != "" || h.Get("X-Ratelimit-Limit-Tokens") != "",
        }
        s.rlMu.Lock()
        s.rl = rl
        s.rlMu.Unlock()
...
func (s *Server) proxyUpstream(w http.ResponseWriter, r *http.Request, path string) {
        ...
        // Capture the upstream rate-limit snapshot from the response headers
        // so the UI can show remaining requests/tokens. The model name comes
        // from the (possibly rewritten) request body.
        modelName, _ := m["model"].(string)
        s.captureRateLimit(resp.Header, modelName)
